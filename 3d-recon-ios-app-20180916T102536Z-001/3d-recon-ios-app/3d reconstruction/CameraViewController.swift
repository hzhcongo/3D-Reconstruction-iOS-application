...
//    var stillImageOutput : AVCaptureStillImageOutput?
    ...
    @IBOutlet weak var uploadingTxt: UILabel!
...
//            captureSession?.sessionPreset = AVCaptureSession.Preset.hd4K3840x2160
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        switch(photoq){
        case 0:
            let xyz = CGRect(x: 0, y: 30, width: 380, height: 610)
            previewLayer!.frame = xyz
        default:
            let xyz = CGRect(x: 0, y: 104, width: 380, height: 461)
            previewLayer!.frame = xyz
        }
        
        ////ALERT DIALOG BOX
        // create the alert
        let alert2 = UIAlertController(title: "Prepare for image-taking", message: "Position the camera at a 20-35 degrees angle and allow as much detail and surface area of the object to be captured. Automatic image-taking will be done after selecting Start.", preferredStyle: UIAlertControllerStyle.alert)
    ...
    @IBAction func takeTopPhoto(_ sender: Any) {
        ...
        ////ALERT DIALOG BOX
        // create the alert
        let alert = UIAlertController(title: "All images taken", message: "Images will now be uploaded to the web server.", preferredStyle: UIAlertControllerStyle.alert)
        // add an action (button)
        alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.default, handler: {
            action in self.uploadToServer(self)
        }))
        // show the alert
...
//            self.captureSession!.stopRunning()
            ...
            self.uploadingTxt.isHidden = false
        }
        ...
        let task = session.dataTask(with: request as URLRequest) {
            ...
            if error == nil {
                DispatchQueue.main.async { // Correct
                    ...
                    self.loaderCircle.isHidden=true
                    self.uploadingTxt.text = "Done!"
                    self.uploadingTxt.frame.origin = CGPoint(x: 141, y: 323)
                }
                ...
                DispatchQueue.main.async { // Correct
                    self.loaderCircle.stopAnimating()
                    self.loaderCircle.isHidden=true
                    self.uploadingTxt.isHidden = true
                }
                
                ///ALERT DIALOG BOX
                // create the alert
                //ttd:if error nil or connection lost?
                let alert = UIAlertController(title: "Error in uploading", message: "An error occured during uploading to " + self.IPaddress + ". Please check if the web and iOS application are connected to the internet, and the IP address is valid.", preferredStyle: UIAlertControllerStyle.alert)
                // add an action (button)
                alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.default, handler: {
                    action in self.outputTxt.text =  String(describing: error)
                    self.outputTxt.isHidden = false
                }))
                // show the alert
                self.present(alert, animated: true, completion: nil)
            }
